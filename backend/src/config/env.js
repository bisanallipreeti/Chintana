...
function validateEnv(config) {
  ...
  if (config.enableBullMq && !config.redisUrl) {
    issues.push(
      "ENABLE_BULLMQ is true but REDIS_URL is not set."
    );
  }
  ...
  if (hasPartialSmtp && !hasCompleteSmtp) {
    issues.push(
      "SMTP configuration is incomplete. Set SMTP_HOST, SMTP_USER, and SMTP_PASS together."
    );
  }
  ...
  if (config.nodeEnv === "production") {
    ...
    if (
      ...
      issues.push(
        "REFRESH_TOKEN_SECRET must be overridden in production."
      );
    }
    ...
    if (
      ...
      issues.push(
        "FORCE_HTTPS=true requires TRUST_PROXY=true when behind a reverse proxy."
      );
    }
  }
...
export const env = {
  // App
  appName:
    process.env.APP_NAME || "chintana-backend",
  ...
  otpTtlMinutes: toNumber(
    process.env.OTP_TTL_MINUTES,
    10
  ),

  passwordResetTtlMinutes: toNumber(
  ...
  groqFallbackModels: toList(
    process.env.GROQ_FALLBACK_MODELS
  ),

  groqMaxTokens: toNumber(
    process.env.GROQ_MAX_TOKENS,
    420
  ),

  groqTemperature: toNumber(
    process.env.GROQ_TEMPERATURE,
    0.2
  ),

  groqTlsRejectUnauthorized: toBoolean(
    process.env.GROQ_TLS_REJECT_UNAUTHORIZED,
    true
  ),

  aiTimeoutMs: toNumber(
    process.env.AI_TIMEOUT_MS,
    12000
  ),

  aiMaxRetries: toNumber(
    process.env.AI_MAX_RETRIES,
    2
  ),

  aiRetryBaseDelayMs: toNumber(
    process.env.AI_RETRY_BASE_DELAY_MS,
    600
  ),

  aiCacheTtlSeconds: toNumber(
    process.env.AI_CACHE_TTL_SECONDS,
    900
  ),

  aiCacheMaxEntries: toNumber(
    process.env.AI_CACHE_MAX_ENTRIES,
    5000
  ),

  aiDailyLimitPerUser: toNumber(
  ...
  enableBullMq: toBoolean(
    process.env.ENABLE_BULLMQ,
    false
  ),

  dailyReminderSweepIntervalMinutes:
    toNumber(
      process.env
        .DAILY_REMINDER_SWEEP_INTERVAL_MINUTES,
      60
    ),
  ...
  smtpTlsRejectUnauthorized: toBoolean(
    process.env
  ...
  mailFrom:
    ...
    "Chintana <no-reply@chintana.app>",
  ...
  mobileScheme:
    process.env.MOBILE_SCHEME || "chintana",
